...
export const config = {
  ...
  generationConcurrency: positiveInt("GENERATION_CONCURRENCY", 6),
  maxActiveGenerationsPerUser: positiveInt("MAX_ACTIVE_GENERATIONS_PER_USER", 6),
  disabledVideoModels: (process.env.DISABLED_VIDEO_MODELS ?? "").split(",").map((model) => model.trim()).filter(Boolean),
  maxActiveUploadsPerUser: boundedInt("MAX_ACTIVE_UPLOADS_PER_USER", 9, 1, 20),
  mediaMaintenanceConcurrency: boundedInt("MEDIA_MAINTENANCE_CONCURRENCY", 3, 1, 6),
  uploadFinalizationConcurrency: boundedInt("UPLOAD_FINALIZATION_CONCURRENCY", 3, 1, 6),
  assetIngestConcurrency: boundedInt("ASSET_INGEST_CONCURRENCY", 3, 1, 6),
  ...
  tosArchiveConcurrency: boundedInt("TOS_ARCHIVE_CONCURRENCY", 4, 1, 6),
  ...
  tosSourceReadConcurrency: boundedInt("TOS_SOURCE_READ_CONCURRENCY", 10, 2, 16),
