...
type CanvasMediaRouteDependencies = {
  ...
  signedObjectUrl: (objectKey: string, options: { fileName: string }) => string;
  cacheControl: string;
};
...
export const createCanvasMediaHandler = (dependencies: CanvasMediaRouteDependencies): express.RequestHandler =>
  (req, res, next) => {
    try {
      ...
      return res.redirect(302, dependencies.signedObjectUrl(asset.objectKey, { fileName: asset.fileName }));
    } catch (error) {
      next(error);
    }
  };
