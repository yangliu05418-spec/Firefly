...
const main = async () => {
  ...
  const imageGeneration = new Queue("image-generation", { connection: redis });
  ...
  try {
    ...
    try {
      const [generationCounts, mediaCounts, previewCounts, imageGenerationCounts] = await Promise.all([
        generation.getJobCounts("wait", "active", "failed"), media.getJobCounts("wait", "active", "failed"), preview.getJobCounts("wait", "active", "failed"), imageGeneration.getJobCounts("wait", "active", "failed")
      ]);
      queueCounts = { generation: generationCounts, media: mediaCounts, preview: previewCounts, imageGeneration: imageGenerationCounts };
      if ((generationCounts.failed ?? 0) + (mediaCounts.failed ?? 0) + (previewCounts.failed ?? 0) + (imageGenerationCounts.failed ?? 0) > 0) reasons.push("failed_jobs_present");
    ...
    database?.close();
    await Promise.allSettled([generation.close(), media.close(), preview.close(), imageGeneration.close()]);
    redis.disconnect();
  }
