...
/** Burger menu links, verbatim from the target's burger.html extraction. */
const BURGER_LINKS = [
  { label: "Home", href: "/" },
  { label: "About", href: "/about" },
] as const;

/** Footer social links, verbatim from the target's burger.html extraction. */
const SOCIAL_LINKS = [
  { label: "Linkedin", href: "https://www.linkedin.com/company/ciridae/" },
  { label: "x", href: "https://x.com/TheCiridae" },
] as const;

/**
 * The logo pieces (data-morph abstract shapes -> data-start real paths),
 * copied verbatim from the target's burger.html extraction. Four pieces,
 * each morphing from a small abstract shape into its slice of the logo.
 */
const LOGO_PIECES: LogoPieceData[] = [
  {
    viewBox: "0 0 1000 1153",
    morph: "M490.5 195C490.5 189.477 494.977 185 500.5 185V185C506.023 185 510.5 189.477 510.5 195V195C510.5 200.523 506.023 205 500.5 205V205C494.977 205 490.5 200.523 490.5 195V195Z",
    start: "M696.243 195.315C696.243 197.944 694.108 200.078 691.478 200.078C597.635 200.078 550.523 200.078 528.091 222.506C505.659 244.934 505.659 292.019 505.659 385.865C505.659 388.495 503.525 390.629 500.895 390.629C498.265 390.629 496.13 388.495 496.13 385.865C496.13 292.019 496.13 244.934 473.718 222.506C451.286 200.078 404.174 200.078 310.311 200.078C307.681 200.078 305.547 197.944 305.547 195.315C305.547 192.685 307.681 190.551 310.311 190.551C404.174 190.551 451.286 190.551 473.718 168.123C496.13 145.695 496.13 98.61 496.13 4.76377C496.13 2.13417 498.265 0 500.895 0C503.525 0 505.659 2.13417 505.659 4.76377C505.659 98.61 505.659 145.695 528.091 168.123C550.523 190.551 597.635 190.551 691.478 190.551C694.108 190.551 696.243 192.685 696.243 195.315Z",
  },
  {
    viewBox: "0 0 1000 1153",
      ...
      <div className="container">
        <div className="burger_body">
          <div className="burger_top grow">
            <Link href="/" aria-current="page" className="burger_logo w-inline-block w--current">
              <LogoPieces key={openCount} pieces={LOGO_PIECES} className="logo-small" />
            </Link>
          </div>
          <div className="burger_list grow">
            {BURGER_LINKS.map((link, i) => (
              <Link
                key={link.href}
                href={link.href}
                aria-current={i === 0 ? "page" : undefined}
                className={`burger_item w-inline-block${i === 0 ? " w--current" : ""} go prevent`}
                style={{ "--delay": `${i * 0.1 + 0.2}s` } as CSSProperties}
              >
                <div className="f-40 caps -ls-06" data-text={link.label} aria-label={link.label}>
            ...
            <div id="w-node-f097604a-6456-bffe-8cd4-420bd162c6a1-d162c691" className="burger_bottom-mid">
              ...
              <div className="v-flex-center-center">
                {SOCIAL_LINKS.map((link) => (
                  <a key={link.href} href={link.href} target="_blank" className="text-link w-inline-block">
                    <LinkAnimation className="caps lh-110 font-mono f-14">{link.label}</LinkAnimation>
                  </a>
                ))}
              </div>
              <div className="dot"></div>
