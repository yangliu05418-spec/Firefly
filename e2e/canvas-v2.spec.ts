...
async function mockAuthenticatedApi(page: Page, options: {
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (path === "/api/generation-capacity" && request.method() === "GET") {
      ...
      return json(route, { active, limit: 10, available: Math.max(0, 10 - active) });
    }
...
test("an archived preview separates the temporary download from the pending TOS download", async ({ page }) => {
  ...
  await mockAuthenticatedApi(page, { videoHistory: [{
    ...
    videoUrl: `/api/generations/${taskId}/media?rev=1`,
    immediateDownloadUrl: `/api/generations/${taskId}/download/temporary`, originalDownloadStatus: "archiving",
...
test("video composer keeps the accepted draft ready for consecutive parallel submissions", async ({ page }) => {
  ...
  await expect(dock.locator(".generation-capacity")).toContainText("2/10 生成中");
  for (let count = 3; count <= 10; count++) {
    await expect(dock.getByRole("button", { name: "生成视频" })).toBeEnabled();
    await dock.getByRole("button", { name: "生成视频" }).click();
    await expect.poll(() => mock.postedGenerations()).toHaveLength(count);
    await expect(dock.locator(".generation-capacity")).toContainText(`${count}/10 生成中`);
  }
