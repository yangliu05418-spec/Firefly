...
type PersistedAsset = { asset: UploadAsset; cachedAt: number };
...
const persistableAsset = (asset: UploadAsset) => Boolean(asset.assetId || asset.uploadId) && asset.progress === 100;
...
export const createComposerDraftCache = (store: ComposerDraftStore, now = () => Date.now()) => {
  ...
  return {
    async read(userId: string, sessionId: string) {
      ...
      let droppedAssets = 0;
      const assets = record.state.assets.filter(({ asset, cachedAt }) => {
        const valid = persistableAsset(asset) && (Boolean(asset.assetId) || now() - cachedAt <= MAX_DIRECT_UPLOAD_AGE_MS);
    ...
    write(userId: string, sessionId: string, state: ComposerDraftState) {
      ...
      return enqueue(key, async () => {
        ...
        const cachedAt = new Map(previous?.state.assets.map((item) => [item.asset.id, item.cachedAt]) ?? []);
        const assets = state.assets.filter(persistableAsset).slice(0, MAX_ASSETS).map((asset) => ({ asset: stripEphemeralAssetFields(asset), cachedAt: cachedAt.get(asset.id) ?? now() }));
