...
export function Composer({ models, compact, sessionId, restore, onRestoreConsumed, onCreated, onImagesGenerated, generationCapacity, admissionConfirmationPending = false, onAdmissionConfirmationChange, onGenerationSettled, destination }: { models: ModelCapability[]; compact: boolean; sessionId: string; restore?: ComposerRestore; onRestoreConsumed?: () => void; onCreated: (task: Task) => void; onImagesGenerated?: (bundle: ImageResultBundle) => void; generationCapacity?: GenerationCapacity | null; admissionConfirmationPending?: boolean; onAdmissionConfirmationChange?: (pending: boolean) => void; onGenerationSettled?: () => void; destination?: { kind: "atlas_project"; projectId: string } }) {
  ...
  useEffect(() => {
    ...
    void composerDraftCache.read(userId, sessionId).then(async (restored) => {
      ...
      setImageResolution(state.imageResolution);
      setImageCount(state.imageCount);
      const cachedAssets = state.assets.map((asset) => asset.snapshotReferenceId
        ? { ...asset, progress: 100 }
        : asset.assetId
          ? { ...asset, progress: 100, status: asset.status ?? "Processing" as const }
          : { ...asset, progress: 100, phase: "verifying" as const });
    ...
    try {
      if (engine === "image") {
        ...
        const references = assets.filter((asset) => asset.type === "image").map((asset) => ({
          ...
          assetId: asset.assetId, snapshotReferenceId: asset.snapshotReferenceId, atlasProjectAssetId: asset.atlasProjectAssetId,
          name: asset.name, type: "image" as const, role: "reference_image" as const,
        }));
        onImagesGenerated?.(pendingImage);
        await api.post<ImageGenResponse>("/api/image-generation", { requestId: pendingImage.id, sessionId, model: imageModelId, ratio: imageRatio, resolution: imageResolution, count: imageCount, prompt: submittedPrompt, editorPrompt, references, destination }, { timeoutMs: 8_000 });
      ...
      const task = await api.post<Task>("/api/generations", { requestId, sessionId, prompt: submittedPrompt, editorPrompt, model: model.id, mode, ratio, resolution, duration, generateAudio: model.supportsAudio && generateAudio, seed, cameraFixed, watermark, outputFormat: "mp4", assets: assets.map((attached) => { const { preview, progress, size, ...asset } = attached; return { ...asset, bindingId: referenceBindingId(attached) }; }), destination }, { timeoutMs: 8_000 });
      ...
      <div className="control-row">
        ...
        {engine === "image" ? (
          <>
            ...
            <div className="control-wrap"><button className="control control--icon" aria-label="选择参考图" onClick={() => { void persistPrivateMediaStorage(); setOpen(open === "library" ? null : "library"); }}>@</button>{open === "library" && <LibraryPanel atlasProjectId={destination?.projectId} add={(asset) => { attachMentionAsset(asset); setOpen(null); }} />}</div>
          </>
        ) : (
          <>
        ...
        {(["omni", "edit", "extend"] as CreationMode[]).includes(mode) && <div className="control-wrap"><button className="control control--icon" aria-label="选择素材" onClick={() => { void persistPrivateMediaStorage(); setOpen(open === "library" ? null : "library"); }}>@</button>{open === "library" && <LibraryPanel atlasProjectId={destination?.projectId} add={(asset) => { attachMentionAsset(asset); setOpen(null); }} />}</div>}
