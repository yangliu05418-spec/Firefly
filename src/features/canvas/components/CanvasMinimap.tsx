/**
 * 小地图：世界范围自适应 + 视口矩形 + 拖拽跳转。
 * 移植自 infinite-canvas（MIT）components/canvas/canvas-mini-map.tsx（颜色映射改自内置节点规格表）。
 */
import { useCallback, useMemo, useRef, useState } from "react";
import type { CanvasNode, CanvasViewportTransform } from "../canvas-types";
import { getNodeSpec } from "../core/nodes";

export function CanvasMinimap({ nodes, viewport, viewportSize, onViewportChange }: {
  nodes: CanvasNode[];
  ...
  const { worldBounds, scale, offset } = useMemo(() => {
    if (!nodes.length) return { worldBounds: { x: -500, y: -500, w: 1000, h: 1000 }, scale: 0.16, offset: { x: 40, y: 0 } };
    let minX = Infinity;
    let minY = Infinity;
    let maxX = -Infinity;
    let maxY = -Infinity;
  ...
  const toMinimap = useCallback(
    (worldX: number, worldY: number) => ({ x: (worldX - worldBounds.x) * scale + offset.x, y: (worldY - worldBounds.y) * scale + offset.y }),
    [offset.x, offset.y, scale, worldBounds.x, worldBounds.y],
  );
  const toWorld = useCallback(
    (minimapX: number, minimapY: number) => ({ x: (minimapX - offset.x) / scale + worldBounds.x, y: (minimapY - offset.y) / scale + worldBounds.y }),
    [offset.x, offset.y, scale, worldBounds.x, worldBounds.y],
  );
  ...
  return (
    <div className="canvas-minimap" style={{ width, height }}>
      <div
        ref={containerRef}
        className="canvas-minimap__surface"
        onPointerDown={(event) => {
      ...
      >
        {nodes.map((node) => {
          ...
          const color = getNodeSpec(node.type).minimapColor ?? "var(--canvas-node-muted)";
          return (
            <div
              key={node.id}
              className="canvas-minimap__node"
              style={{ left: pos.x, top: pos.y, width: Math.max(node.width * scale, 2), height: Math.max(node.height * scale, 2), backgroundColor: color }}
            />
          );
        })}
        <div className="canvas-minimap__viewport" style={{ left: viewportRect.x, top: viewportRect.y, width: viewportRect.w, height: viewportRect.h }} />
      </div>
    </div>
  );
}
