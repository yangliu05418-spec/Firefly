...
export function CanvasMontage({ canvasId, initialAssets, allAssets, onClose, onComplete }: { canvasId: string; initialAssets: CanvasProjectAsset[]; allAssets: CanvasProjectAsset[]; onClose: () => void; onComplete: (asset: CanvasProjectAsset) => void }) {
  ...
  const moveTo = (from: number, target: number) => { if (from === target || from < 0 || target < 0) return; const video = [...timeline.video]; const [clip] = video.splice(from, 1); video.splice(target, 0, clip); update({ ...timeline, video }); setActiveClip(target); };
  ...
  return <div className="canvas-v2-modal canvas-v2-montage-modal" role="dialog" aria-modal="true" aria-label="Montage"><section className="canvas-v2-montage">
    ...
    <div className="canvas-v2-montage__timeline">
      ...
      <div className="canvas-v2-montage__clips">{timeline.video.map((clip, index) => { const asset = allAssets.find((item) => item.id === clip.projectAssetId); return <article key={clip.id} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = "move"; }} onDrop={(event) => { event.preventDefault(); if (draggedClip !== null) moveTo(draggedClip, index); setDraggedClip(null); }} className={`${index === activeClip ? "active" : ""}${index === draggedClip ? " dragging" : ""}`} onClick={() => setActiveClip(index)}><span className="canvas-v2-montage__drag" draggable onDragStart={(event) => { setDraggedClip(index); event.dataTransfer.effectAllowed = "move"; }} onDragEnd={() => setDraggedClip(null)}><GripVertical /></span><div><b>{asset?.title ?? "视频片段"}</b><small>{Math.round((clip.durationMs - clip.trimStartMs - clip.trimEndMs) / 100) / 10}s</small></div><label title="左裁剪"><Scissors /><input type="number" min={0} max={clip.durationMs - clip.trimEndMs - 100} step={100} value={clip.trimStartMs} onChange={(event) => patchClip(clip.id, { trimStartMs: Number(event.target.value) })} /></label><label title="右裁剪"><input type="number" min={0} max={clip.durationMs - clip.trimStartMs - 100} step={100} value={clip.trimEndMs} onChange={(event) => patchClip(clip.id, { trimEndMs: Number(event.target.value) })} /></label><button onClick={(event) => { event.stopPropagation(); patchClip(clip.id, { muted: !clip.muted }); }}>{clip.muted ? <VolumeX /> : <Volume2 />}</button><button onClick={(event) => { event.stopPropagation(); move(index, -1); }}>←</button><button onClick={(event) => { event.stopPropagation(); move(index, 1); }}>→</button><button onClick={(event) => { event.stopPropagation(); remove(clip.id); }}><Trash2 /></button></article>; })}</div>
