...
import { TextPlayground } from "./features/text-generation/TextPlayground";
import { clearTextDrafts, rememberTextMode, textModeSelected } from "./features/text-generation/text-cache";
...
import { createSessionRecoverably, hasActiveStudioWork, isAmbiguousSubmissionFailure, isVideoTaskActive, mergeGenerationHistoryHead, mergeSessionHistoryHead, replaceSessionSnapshot, selectSessionSnapshot, upsertStudioItem } from "./studio-sync";
...
const reportMediaEvent = (taskId: string, event: MediaEventName, startedAt: number, video?: HTMLVideoElement, bufferingMs?: number, downloadTarget?: "temporary_original" | "tos_original") => {
...
function TaskCard({ task: inputTask, models, eager, now, onDelete, onReedit, reeditBusy = false, canDelete = false }: { task: Task; models: ModelCapability[]; eager: boolean; now: number; onDelete: (task: Task) => void; onReedit: (kind: "video", id: string) => void; reeditBusy?: boolean; canDelete?: boolean }) {
  ...
  const temporaryAvailable = task.temporaryOriginalStatus === "ready" && Boolean(task.temporaryVideoUrl);
  const temporaryExpired = task.temporaryOriginalStatus === "expired";
  const expired = false;
  ...
  const reportTaskMediaEvent = (event: MediaEventName, video?: HTMLVideoElement, bufferingMs?: number, downloadTarget?: "temporary_original" | "tos_original") => reportMediaEvent(task.id, event, loadStartedAt.current, video, bufferingMs, downloadTarget);
  useEffect(() => {
...
function AssetPreview({ task, close, onDelete, initialTime, onPosition }: { task: Task; close: () => void; onDelete: (task: Task) => void; initialTime: number; onPosition: (time: number) => void }) {
  ...
  return <div className="asset-preview-backdrop" role="dialog" aria-modal="true" aria-labelledby="asset-preview-title" onClick={close}>
    <div ref={dialogRef} className="asset-preview" onKeyDown={handleDialogKeyDown} onClick={(event) => event.stopPropagation()}>
      ...
      <footer><span>{downloadNotice || `${new Date(task.createdAt).toLocaleString("zh-CN")} · ${task.resolution} · ${task.ratio}`}</span><div><CaseIdButton task={task} /><button onClick={() => { close(); onDelete(task); }}><Trash2 /> 删除</button>{task.immediateDownloadUrl && <a href={task.immediateDownloadUrl} target="_blank" rel="noreferrer" title="从模型临时源立即下载，速度可能较慢" onClick={() => { setDownloadNotice("已开始从临时源下载，速度可能较慢"); reportMediaEvent(task.id, "download_click", startedAt.current, undefined, undefined, "temporary_original"); }}><Download /> 立即下载 · 较慢</a>}{task.downloadUrl ? <a href={task.downloadUrl} target="_blank" rel="noreferrer" title="从北京 TOS 高速下载" onClick={() => { setDownloadNotice("TOS 高速原片已交给浏览器下载器"); reportMediaEvent(task.id, "download_click", startedAt.current, undefined, undefined, "tos_original"); }}><Download /> 高速下载</a> : <button disabled><Download /> 高速下载归档中</button>}</div></footer>
    </div>
  </div>;
}
...
function AssetArchive({ tasks, imageResults, models, onCreate, onDelete, onRemoveImage, onReedit, reeditBusyId, onInsertCanvas }: { tasks: Task[]; imageResults: ImageResultBundle[]; models: ModelCapability[]; onCreate: () => void; onDelete: (task: Task) => void; onRemoveImage: (id: string) => void; onReedit: (kind: "video" | "image", id: string) => void; reeditBusyId: string | null; onInsertCanvas: (target: { kind: "video"; task: Task } | { kind: "image"; asset: LibraryAsset } | { kind: "generated"; mediaId: string; title: string }) => void }) {
  ...
  const announceDownload = (task: Task) => {
    reportMediaEvent(task.id, "download_click", Date.now(), undefined, undefined, "tos_original");
  ...
  const copyDownloadEntry = async () => {
    ...
    const url = downloadNotice.task.downloadUrl;
    try {
...
function Studio({ user, route, navigate, logout }: { user: SessionUser; route: string; navigate: (path: string) => void; logout: () => void }) {
  ...
  const [textSelection, setTextSelection] = useState<{ sessionId: string; enabled: boolean } | null>(null);
  const [mediaEngineRequest, setMediaEngineRequest] = useState<{ engine: "video" | "image"; nonce: number; sessionId: string }>();
  ...
  const textActive = textSelection?.sessionId === activeSessionId ? textSelection.enabled : textModeSelected(user.id, activeSessionId);
  const selectTextMode = (enabled: boolean, sessionId = activeSessionId) => { rememberTextMode(user.id, sessionId, enabled); setTextSelection({ sessionId, enabled }); };
  ...
  const applyReedit = async (payload: ComposerRestorePayload, destination: CreationSession, backup?: ReeditUndo) => {
    selectTextMode(false, destination.id);
  ...
  const activateAtlas = () => {
    window.location.assign("/studio/atlas/");
  };
  ...
  const confirmDeleteSession = async () => { if (!sessionDeleteTarget) return; setSessionBusy(true); try { await api.delete(`/api/creation-sessions/${encodeURIComponent(sessionDeleteTarget.id)}`); void composerDraftCache.clearSession(user.id, sessionDeleteTarget.id); void clearTextDrafts(user.id, sessionDeleteTarget.id).catch(() => undefined); rememberTextMode(user.id, sessionDeleteTarget.id, false); const remaining = sessions.filter((session) => session.id !== sessionDeleteTarget.id); setSessions(remaining); setSessionDeleteTarget(null); if (sessionDeleteTarget.id === activeSessionId) { if (remaining[0]) await openSession(remaining[0]); else await createSession(); } } catch { setSyncIssue(true); } finally { setSessionBusy(false); } };
