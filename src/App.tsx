...
function AssetPreview({ task, close, onDelete, initialTime, onPosition }: { task: Task; close: () => void; onDelete: (task: Task) => void; initialTime: number; onPosition: (time: number) => void }) {
  ...
  return <div className="asset-preview-backdrop" role="dialog" aria-modal="true" aria-labelledby="asset-preview-title" onClick={close}>
    <div ref={dialogRef} className="asset-preview" onKeyDown={handleDialogKeyDown} onClick={(event) => event.stopPropagation()}>
      ...
      <footer><span>{downloadNotice || `${new Date(task.createdAt).toLocaleString("zh-CN")} · ${task.resolution} · ${task.ratio}`}</span><div><CaseIdButton task={task} /><button onClick={() => { close(); onDelete(task); }}><Trash2 /> 删除</button>{task.immediateDownloadUrl && <a href={task.immediateDownloadUrl} target="_blank" rel="noreferrer" title="从模型临时源立即下载，速度可能较慢" onClick={() => { setDownloadNotice("已开始从临时源下载，速度可能较慢"); reportMediaEvent(task.id, "download_click", startedAt.current); }}><Download /> 立即下载 · 较慢</a>}{task.downloadUrl ? <a href={task.downloadUrl} target="_blank" rel="noreferrer" title="从北京 TOS 高速下载" onClick={() => { setDownloadNotice("TOS 高速原片已交给浏览器下载器"); reportMediaEvent(task.id, "download_click", startedAt.current); }}><Download /> 高速下载</a> : <button disabled><Download /> 高速下载归档中</button>}</div></footer>
    </div>
  </div>;
}
...
function Studio({ user, route, navigate, logout }: { user: SessionUser; route: string; navigate: (path: string) => void; logout: () => void }) {
  ...
  const refresh = async () => {
    ...
    const [taskResult, imageResult, capacityResult] = await Promise.allSettled([listGenerationHistory(activeSessionId), api.get<ImageResultBundle[]>(`/api/image-generations${query}`), api.get<GenerationCapacity>("/api/generation-capacity")]);
