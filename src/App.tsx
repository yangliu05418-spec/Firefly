...
function ImageAssetManager({ onInsertCanvas }: { onInsertCanvas: (asset: LibraryAsset) => void }) {
  ...
  const uploadImages = async (files?: FileList | null) => {
    ...
    try {
      ...
      if (created.length) { const ordered = [...created].reverse(); setAssets((current) => [...ordered, ...current]); void assetMetadataCache.merge(userId, ordered); setNotice(`${created.length} 张图片已上传，生成引用正在后台准备${normalizedCount ? `，${normalizedCount} 张已自动补白` : ""}`); }
