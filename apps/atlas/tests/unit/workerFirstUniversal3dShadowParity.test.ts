...
import {
  handleRunWorkerFirstUniversal3dShadowParity,
  type WorkerFirstUniversal3dShadowParityDeps,
} from '../../src/services/aiTools/workerFirstUniversal3dShadowParity';
import {
...
const mainFingerprint: FrameFingerprint = {
  ...
  nonBlankRatio: 1,
  alphaCoverage: 1,
  avgRgb: { r: 36.6, g: 56.8, b: 83.3 },
  meanLuma: 54.4,
  colorRange: { r: 236, g: 224, b: 204, luma: 224.6 },
};
...
function createDeps(options: {
...
} = {}): WorkerFirstUniversal3dShadowParityDeps {
  ...
  return {
    materializeAndCaptureMainFixture: vi.fn(async () => options.materializeResult ?? {
      ...
      data: {
        ...
        fixture: {
          ...
          timelineSignals: ['3d', 'cad', 'gaussian', 'image', 'model', 'solid', 'text'],
        },
      },
    }),
    ...
    renderWorkerShadowFingerprint: vi.fn(async () => ({
      renderer: 'worker-offscreen-2d-universal-3d-gaussian-cad',
...
describe('worker-first universal 3D/Gaussian/CAD shadow parity runner', () => {
  afterEach(() => {
    clearWorkerFirstProofCapturesForTests();
    clearWorkerFirstCounterSourcesForTests();
    vi.restoreAllMocks();
  ...
  it('materializes the fixture, captures main fingerprints, renders worker shadow samples, and records parity', async () => {
    ...
    const result = await handleRunWorkerFirstUniversal3dShadowParity({
      sampleWidth: 12,
      sampleHeight: 10,
      settleMs: 80,
    }, deps);
    ...
    expect(counters.presentedFrameId).toBe('universal-3d-gaussian-cad-worker-shadow:2');
  });
  ...
  it('rejects caller-supplied proof fields instead of recording spoofed parity', async () => {
    for (const args of [
      ...
      const result = await handleRunWorkerFirstUniversal3dShadowParity(args, createDeps());
  ...
  it('does not run shadow rendering when fixture materialization fails', async () => {
    ...
    const result = await handleRunWorkerFirstUniversal3dShadowParity({}, deps);
  ...
  it('returns failures when main capture is missing or worker parity is outside thresholds', async () => {
    const deps = createDeps({
      captureResults: [
        { success: false, error: 'missing cad signal' },
        {
          success: true,
          data: {
            sampleTimeSeconds: 1,
    ...
    expect(result.data).toMatchObject({
      failures: [
        { sampleTimeSeconds: 0, error: 'missing cad signal' },
        { sampleTimeSeconds: 1 },
        { sampleTimeSeconds: 2 },
      ],
    });
