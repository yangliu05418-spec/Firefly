...
import {
  handleRunWorkerFirstSolidTextImageShadowParity,
  type WorkerFirstSolidTextImageShadowParityDeps,
} from '../../src/services/aiTools/workerFirstSolidTextImageShadowParity';
import {
...
function createDeps(options: {
...
} = {}): WorkerFirstSolidTextImageShadowParityDeps {
  ...
  return {
    ...
    renderWorkerShadowFingerprint: vi.fn(async () => ({
      renderer: 'worker-offscreen-2d-solid-text-image',
...
describe('worker-first solid/text/image shadow parity runner', () => {
  afterEach(() => {
    clearWorkerFirstProofCapturesForTests();
    clearWorkerFirstCounterSourcesForTests();
    vi.restoreAllMocks();
  ...
  it('materializes the fixture, captures main fingerprints, renders worker shadow samples, and records parity', async () => {
    ...
    const result = await handleRunWorkerFirstSolidTextImageShadowParity({
      sampleWidth: 12,
      sampleHeight: 10,
    }, deps);
    ...
    expect(counters.presentedFrameId).toBe('solid-text-image-worker-shadow:1');
  });
  ...
  it('rejects caller-supplied proof fields instead of recording spoofed parity', async () => {
    for (const args of [
      ...
      const result = await handleRunWorkerFirstSolidTextImageShadowParity(args, createDeps());
  ...
  it('does not run shadow rendering when fixture materialization fails', async () => {
    ...
    const result = await handleRunWorkerFirstSolidTextImageShadowParity({}, deps);
