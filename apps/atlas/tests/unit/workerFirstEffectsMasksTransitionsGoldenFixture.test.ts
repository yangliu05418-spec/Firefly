...
import {
  handleRunWorkerFirstEffectsMasksTransitionsGoldenFixture,
  type WorkerFirstEffectsMasksTransitionsGoldenFixtureDeps,
} from '../../src/services/aiTools/workerFirstEffectsMasksTransitionsGoldenFixture';

function createDeps(options: {
  readonly materializeResult?: ToolResult;
  readonly captureResults?: readonly ToolResult[];
} = {}): WorkerFirstEffectsMasksTransitionsGoldenFixtureDeps {
...
describe('worker-first effects/masks/transitions golden fixture runner', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it('materializes the fixture and captures every manifest sample through the golden bridge', async () => {
    ...
    const result = await handleRunWorkerFirstEffectsMasksTransitionsGoldenFixture({
      sampleWidth: 12,
      sampleHeight: 10,
    }, deps);
  ...
  it('rejects project, proof, and fixture override fields', async () => {
    const wrongProject = await handleRunWorkerFirstEffectsMasksTransitionsGoldenFixture({
    ...
    for (const args of [
      ...
      const result = await handleRunWorkerFirstEffectsMasksTransitionsGoldenFixture(args, createDeps());
      expect(result.success).toBe(false);
      expect(result.error).toContain('controlled by the effects-masks-transitions manifest');
    }
    ...
    for (const args of [
      ...
      const result = await handleRunWorkerFirstEffectsMasksTransitionsGoldenFixture(args, createDeps());
      expect(result.success).toBe(false);
      expect(result.error).toContain('controlled by the runner');
    }
  });
  ...
  it('does not capture when fixture materialization fails', async () => {
    ...
    const result = await handleRunWorkerFirstEffectsMasksTransitionsGoldenFixture({}, deps);
