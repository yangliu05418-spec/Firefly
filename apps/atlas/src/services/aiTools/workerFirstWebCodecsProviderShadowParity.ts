...
import {
  materializeWorkerFirstWebCodecsProviderShadowFixture,
} from './workerFirstWebCodecsProviderGoldenFixture';
import {
...
export interface WorkerFirstWebCodecsProviderShadowParityDeps {
  readonly materializeMainFixture: (args: Record<string, unknown>) => Promise<ToolResult>;
...
async function materializeWebCodecsProviderShadowFixture(args: Record<string, unknown>): Promise<ToolResult> {
  const restoreFlags = enableWebCodecsFixtureFlags();
  const endMutation = beginTimelineCanvasSmokeMutation();
  try {
    return await materializeWorkerFirstWebCodecsProviderShadowFixture(args);
  } finally {
    restoreFlags();
    endMutation();
  }
...
const DEFAULT_DEPS: WorkerFirstWebCodecsProviderShadowParityDeps = {
  materializeMainFixture: (args) => materializeWebCodecsProviderShadowFixture(args),
...
function readNumberArg(args: Record<string, unknown>, key: string, fallback: number, min: number, max: number): number {
  const value = args[key];
  const numeric = typeof value === 'number' && Number.isFinite(value) ? value : fallback;
  return Math.max(min, Math.min(max, numeric));
}

function readSampleDimension(args: Record<string, unknown>, key: 'sampleWidth' | 'sampleHeight'): number | undefined {
...
function createShadowPlan(args: Record<string, unknown>, sampleTimeSeconds: number): WorkerFirstSolidTextImageShadowRenderPlan {
  ...
  const durationSeconds = readNumberArg(args, 'durationSeconds', 2.25, minDuration, 30);
  return { projectId: PROJECT_ID, width, height, durationSeconds, sampleTimeSeconds };
}

function captureArgsForSample(args: Record<string, unknown>, sampleTimeSeconds: number): Record<string, unknown> {
  return {
...
export async function handleRunWorkerFirstWebCodecsProviderShadowParity(
  args: Record<string, unknown>,
  deps: WorkerFirstWebCodecsProviderShadowParityDeps = DEFAULT_DEPS,
): Promise<ToolResult> {
  if (args.projectId !== undefined && args.projectId !== PROJECT_ID) {
    return {
      success: false,
      error: 'This worker-shadow parity runner only supports the webcodecs-provider golden fixture.',
  ...
  const fixtureResult = await deps.materializeMainFixture({
    ...args,
    projectId: PROJECT_ID,
  });
  ...
  for (const sampleTimeSeconds of manifestSampleTimes()) {
    ...
    try {
      const workerResult = await deps.renderWorkerShadowFingerprint(createShadowPlan(args, sampleTimeSeconds), fingerprintOptions);
      const comparison = compareFrameFingerprints(mainFingerprint, workerResult.fingerprint, WORKER_FIRST_STATIC_2D_SHADOW_THRESHOLDS);
      recordWorkerFirstShadowParitySample({
        projectId: PROJECT_ID,
        sampleTimeSeconds,
        mainFingerprint,
      ...
      if (!comparison.passed) {
        failures.push({ sampleTimeSeconds, error: comparison.failures.join(', ') });
      }
    } catch (error) {
      failures.push({ sampleTimeSeconds, error: error instanceof Error ? error.message : String(error) });
    }
  }
  recordWorkerFirstShadowParityRunCounters({
    projectId: PROJECT_ID,
  ...
  return failures.length > 0
    ? { success: false, error: 'One or more webcodecs-provider worker-shadow parity samples failed.', data }
    : { success: true, data };
}
