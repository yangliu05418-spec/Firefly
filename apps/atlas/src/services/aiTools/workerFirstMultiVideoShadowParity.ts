...
} from './workerFirstMultiVideoGoldenFixture';
import {
...
export interface WorkerFirstMultiVideoShadowParityDeps {
...
const DEFAULT_DEPS: WorkerFirstMultiVideoShadowParityDeps = {
  materializeAndCaptureMainFixture: (args) => handleRunWorkerFirstMultiVideoGoldenFixture(args),
...
function createShadowPlan(args: Record<string, unknown>, sampleTimeSeconds: number): WorkerFirstSolidTextImageShadowRenderPlan {
  ...
  const durationSeconds = readNumberArg(args, 'durationSeconds', 6, minDuration, 60);
  return {
...
export async function handleRunWorkerFirstMultiVideoShadowParity(
  args: Record<string, unknown>,
  deps: WorkerFirstMultiVideoShadowParityDeps = DEFAULT_DEPS,
): Promise<ToolResult> {
  if (args.projectId !== undefined && args.projectId !== PROJECT_ID) {
    return {
      success: false,
      error: 'This worker-shadow parity runner only supports the multi-video golden fixture.',
      data: {
        projectId: PROJECT_ID,
      },
    };
  ...
  return failures.length > 0
    ? {
        success: false,
        error: 'One or more multi-video worker-shadow parity samples failed.',
        data,
      }
    : {
        success: true,
