...
} from './workerFirstMultiTargetOutputSliceGoldenFixture';
import {
...
export interface WorkerFirstMultiTargetOutputSliceShadowParityDeps {
...
const DEFAULT_DEPS: WorkerFirstMultiTargetOutputSliceShadowParityDeps = {
  materializeAndCaptureMainFixture: (args) => handleRunWorkerFirstMultiTargetOutputSliceGoldenFixture(args),
...
function createShadowPlan(args: Record<string, unknown>, sampleTimeSeconds: number): WorkerFirstSolidTextImageShadowRenderPlan {
  ...
  const durationSeconds = readNumberArg(args, 'durationSeconds', 3.25, minDuration, 30);
  return {
...
export async function handleRunWorkerFirstMultiTargetOutputSliceShadowParity(
  args: Record<string, unknown>,
  deps: WorkerFirstMultiTargetOutputSliceShadowParityDeps = DEFAULT_DEPS,
): Promise<ToolResult> {
  if (args.projectId !== undefined && args.projectId !== PROJECT_ID) {
    return {
      success: false,
      error: 'This worker-shadow parity runner only supports the multi-target-output-slice golden fixture.',
      data: {
        projectId: PROJECT_ID,
      },
    };
  ...
  return failures.length > 0
    ? {
        success: false,
        error: 'One or more multi-target-output-slice worker-shadow parity samples failed.',
        data,
      }
    : {
        success: true,
