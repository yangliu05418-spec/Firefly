...
} from './workerFirstHtmlProviderGoldenFixture';
import {
...
export interface WorkerFirstHtmlProviderShadowParityDeps {
...
const DEFAULT_DEPS: WorkerFirstHtmlProviderShadowParityDeps = {
  materializeAndCaptureMainFixture: (args) => handleRunWorkerFirstHtmlProviderGoldenFixture(args),
...
function createShadowPlan(args: Record<string, unknown>, sampleTimeSeconds: number): WorkerFirstSolidTextImageShadowRenderPlan {
  ...
  const durationSeconds = readNumberArg(args, 'durationSeconds', 3, minDuration, 30);
...
export async function handleRunWorkerFirstHtmlProviderShadowParity(
  args: Record<string, unknown>,
  deps: WorkerFirstHtmlProviderShadowParityDeps = DEFAULT_DEPS,
): Promise<ToolResult> {
  if (args.projectId !== undefined && args.projectId !== PROJECT_ID) {
    return {
      success: false,
      error: 'This worker-shadow parity runner only supports the html-provider-fallback golden fixture.',
  ...
  return failures.length > 0
    ? { success: false, error: 'One or more html-provider-fallback worker-shadow parity samples failed.', data }
    : { success: true, data };
}
