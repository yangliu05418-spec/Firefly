...
} from './workerFirstBakeGoldenFixture';
import {
...
export interface WorkerFirstBakeShadowParityDeps {
...
const DEFAULT_DEPS: WorkerFirstBakeShadowParityDeps = {
  materializeAndCaptureMainFixture: (args) => handleRunWorkerFirstBakeGoldenFixture(args),
...
export async function handleRunWorkerFirstBakeShadowParity(
  args: Record<string, unknown>,
  deps: WorkerFirstBakeShadowParityDeps = DEFAULT_DEPS,
): Promise<ToolResult> {
  if (args.projectId !== undefined && args.projectId !== PROJECT_ID) {
    return {
      success: false,
      error: 'This worker-shadow parity runner only supports the bake golden fixture.',
      data: {
        projectId: PROJECT_ID,
      },
    };
  ...
  for (const sampleTimeSeconds of manifestSampleTimes()) {
    ...
    if (!captureResult.success || !mainFingerprint) {
      failures.push({
        sampleTimeSeconds,
        error: captureResult.error ?? 'Main renderer bake capture did not return a fingerprint.',
      });
      continue;
    }
  ...
  return failures.length > 0
    ? {
        success: false,
        error: 'One or more bake worker-shadow parity samples failed.',
        data,
      }
    : {
        success: true,
