...
} from './workerFirstNestedCompsGoldenFixture';
import {
...
export interface WorkerFirstNestedCompsShadowParityDeps {
...
const DEFAULT_DEPS: WorkerFirstNestedCompsShadowParityDeps = {
  materializeAndCaptureMainFixture: (args) => handleRunWorkerFirstNestedCompsGoldenFixture(args),
...
function createShadowPlan(args: Record<string, unknown>, sampleTimeSeconds: number): WorkerFirstSolidTextImageShadowRenderPlan {
  ...
  const durationSeconds = readNumberArg(args, 'durationSeconds', 3.25, minDuration, 10);
  return {
...
export async function handleRunWorkerFirstNestedCompsShadowParity(
  args: Record<string, unknown>,
  deps: WorkerFirstNestedCompsShadowParityDeps = DEFAULT_DEPS,
): Promise<ToolResult> {
  if (args.projectId !== undefined && args.projectId !== PROJECT_ID) {
    return {
      success: false,
      error: 'This worker-shadow parity runner only supports the nested-comps golden fixture.',
      data: {
        projectId: PROJECT_ID,
      },
    };
  ...
  return failures.length > 0
    ? {
        success: false,
        error: 'One or more nested-comps worker-shadow parity samples failed.',
        data,
      }
    : {
        success: true,
