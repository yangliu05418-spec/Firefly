...
} from './workerFirstJpegProxyGoldenFixture';
import {
...
export interface WorkerFirstJpegProxyShadowParityDeps {
...
const DEFAULT_DEPS: WorkerFirstJpegProxyShadowParityDeps = {
  materializeAndCaptureMainFixture: (args) => handleRunWorkerFirstJpegProxyGoldenFixture(args),
...
export async function handleRunWorkerFirstJpegProxyShadowParity(
  args: Record<string, unknown>,
  deps: WorkerFirstJpegProxyShadowParityDeps = DEFAULT_DEPS,
): Promise<ToolResult> {
  if (args.projectId !== undefined && args.projectId !== PROJECT_ID) {
    return {
      success: false,
      error: 'This worker-shadow parity runner only supports the jpeg-proxy golden fixture.',
      data: {
        projectId: PROJECT_ID,
      },
    };
  ...
  return failures.length > 0
    ? {
        success: false,
        error: 'One or more jpeg-proxy worker-shadow parity samples failed.',
        data,
      }
    : {
        success: true,
