...
} from './workerFirstEffectsMasksTransitionsGoldenFixture';
import {
...
export interface WorkerFirstEffectsMasksTransitionsShadowParityDeps {
...
const DEFAULT_DEPS: WorkerFirstEffectsMasksTransitionsShadowParityDeps = {
  materializeAndCaptureMainFixture: (args) => handleRunWorkerFirstEffectsMasksTransitionsGoldenFixture(args),
...
function createShadowPlan(args: Record<string, unknown>, sampleTimeSeconds: number): WorkerFirstSolidTextImageShadowRenderPlan {
  ...
  const durationSeconds = readNumberArg(args, 'durationSeconds', 2, minDuration, 10);
  return {
...
export async function handleRunWorkerFirstEffectsMasksTransitionsShadowParity(
  args: Record<string, unknown>,
  deps: WorkerFirstEffectsMasksTransitionsShadowParityDeps = DEFAULT_DEPS,
): Promise<ToolResult> {
  if (args.projectId !== undefined && args.projectId !== PROJECT_ID) {
    return {
      success: false,
      error: 'This worker-shadow parity runner only supports the effects-masks-transitions golden fixture.',
      data: {
        projectId: PROJECT_ID,
      },
    };
  ...
  return failures.length > 0
    ? {
        success: false,
        error: 'One or more effects-masks-transitions worker-shadow parity samples failed.',
        data,
      }
    : {
        success: true,
