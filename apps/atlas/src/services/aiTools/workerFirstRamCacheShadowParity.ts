...
} from './workerFirstRamCacheGoldenFixture';
import {
...
export interface WorkerFirstRamCacheShadowParityDeps {
...
const DEFAULT_DEPS: WorkerFirstRamCacheShadowParityDeps = {
  materializeAndCaptureMainFixture: (args) => handleRunWorkerFirstRamCacheGoldenFixture(args),
...
function createShadowPlan(args: Record<string, unknown>, sampleTimeSeconds: number): WorkerFirstSolidTextImageShadowRenderPlan {
  ...
  const durationSeconds = readNumberArg(args, 'durationSeconds', 1.35, minDuration, 30);
  return {
...
export async function handleRunWorkerFirstRamCacheShadowParity(
  args: Record<string, unknown>,
  deps: WorkerFirstRamCacheShadowParityDeps = DEFAULT_DEPS,
): Promise<ToolResult> {
  if (args.projectId !== undefined && args.projectId !== PROJECT_ID) {
    return {
      success: false,
      error: 'This worker-shadow parity runner only supports the ram-cache golden fixture.',
      data: {
        projectId: PROJECT_ID,
      },
    };
  ...
  for (const sampleTimeSeconds of manifestSampleTimes()) {
    ...
    if (!captureResult.success || !mainFingerprint) {
      failures.push({
        sampleTimeSeconds,
        error: captureResult.error ?? 'Main renderer cache capture did not return a fingerprint.',
      });
      continue;
    }
  ...
  return failures.length > 0
    ? {
        success: false,
        error: 'One or more ram-cache worker-shadow parity samples failed.',
        data,
      }
    : {
        success: true,
